...
public class Player2PlacementController {
    ...
    private void handlePlaceShipClick(int row, int col) {
        ...
        boolean placed = gameState.getPlayer2Board().placeShip(newShip, row, col, isHorizontal);
    ...
    private void updateGridVisual() {
        for (Node node : playerGrid.getChildren()) {
            ...
            Cell cell = gameState.getPlayer2Board().getCell(row, col);
            Rectangle rect = (Rectangle) node;

            if (cell.hasShip()) {
                rect.setFill(Color.DARKRED);
            } else {
                rect.setFill(Color.web("#F08080"));
            }
        }
    }
    ...
    private void randomizeShips() {
        // Clear Player 2 board, not Player 1
        gameState.getPlayer2Board().clearShips();
        shipsPlaced = 0;
        ...
        for (Ship.ShipType type : shipsToPlace) {
            ...
            while (remain > 0 && tries < 2000) {
                ...
                if (gameState.getPlayer2Board().placeShip(newShip, row, col, horizontal)) {
                    remain--;
                    shipsPlaced++;
                }
                tries++;
